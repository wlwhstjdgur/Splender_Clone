...
namespace Unity.InferenceEngine
{
    partial class CPUBackend
    {
        ...
        [BurstCompile(OptimizeFor = OptimizeFor.Performance, FloatMode = FloatMode.Default, FloatPrecision = FloatPrecision.Standard, CompileSynchronously = true)]
        internal unsafe struct ScatterElementsFloatFastJob : IJobParallelFor, IJobResourceDeclarationXBO
        {
            public int outAxisSize;
            public int indicesAxisElementStride;
            public int outAxisElementStride;
            public int indicesAxisMinusOneElementStride;
            public Layers.ScatterReductionMode reductionType;
            public ReadOnlyMemResource X { get; set; } float* Xptr => (float*)X.ptr;
            public ReadOnlyMemResource B { get; set; } int* Bptr => (int*)B.ptr;
            public ReadWriteMemResource O { get; set; } float* Optr => (float*)O.ptr;
            ...
            public void Execute(int threadIdx)
            {
                ...
                var outIdx = outerMostElementNum * outAxisSize * outAxisElementStride + index * outAxisElementStride + trailingOffset;

                if (reductionType == Layers.ScatterReductionMode.None)
                    Optr[outIdx] = Xptr[threadIdx];
                else if (reductionType == Layers.ScatterReductionMode.Add)
                    Optr[outIdx] += Xptr[threadIdx];
                else if (reductionType == Layers.ScatterReductionMode.Mul)
                    Optr[outIdx] *= Xptr[threadIdx];
                else if (reductionType == Layers.ScatterReductionMode.Max)
                    Optr[outIdx] = math.max(Optr[outIdx], Xptr[threadIdx]);
                else if (reductionType == Layers.ScatterReductionMode.Min)
                    Optr[outIdx] = math.min(Optr[outIdx], Xptr[threadIdx]);
            }
        }

        [BurstCompile(OptimizeFor = OptimizeFor.Performance, FloatMode = FloatMode.Default, FloatPrecision = FloatPrecision.Standard, CompileSynchronously = true)]
        internal unsafe struct ScatterElementsIntFastJob : IJobParallelFor, IJobResourceDeclarationXBO
        {
            public int outAxisSize;
            public int indicesAxisElementStride;
            public int outAxisElementStride;
        ...
        [BurstCompile(OptimizeFor = OptimizeFor.Performance, FloatMode = FloatMode.Default, FloatPrecision = FloatPrecision.Standard, CompileSynchronously = true)]
        internal unsafe struct ScatterElementsFloatJob : IJobParallelFor, IJobResourceDeclarationXBO
        {
            public int outAxisSize;
            public int posAxis;
            public int rank;
            ...
            public void Execute(int threadIdx)
            {
                ...
                if (reductionType == Layers.ScatterReductionMode.None)
                    ...
                    Optr[(int)outLinearIdx] = math.max(Optr[(int)outLinearIdx], Xptr[threadIdx]);
                else if (reductionType == Layers.ScatterReductionMode.Min)
                    Optr[(int)outLinearIdx] = math.min(Optr[(int)outLinearIdx], Xptr[threadIdx]);
            }
        }

        [BurstCompile(OptimizeFor = OptimizeFor.Performance, FloatMode = FloatMode.Default, FloatPrecision = FloatPrecision.Standard, CompileSynchronously = true)]
        internal unsafe struct ScatterElementsIntJob : IJobParallelFor, IJobResourceDeclarationXBO
        {
            public int outAxisSize;
            public int posAxis;
            public int rank;
