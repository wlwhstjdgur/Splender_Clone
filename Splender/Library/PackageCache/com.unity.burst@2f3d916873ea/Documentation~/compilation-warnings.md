...
You can apply `IgnoreWarning` to the following:

* **Methods**: Suppress the warning for that method when Burst compiles it.
* **`[BurstDiscard]` methods**: Suppress the `BC1371` discarded-call warning for the calls discarded along with the method (its arguments and any calls nested within them) at every call site (see [BC1371](#bc1371) below). This avoids having to annotate every caller.
...
If the same discarded method is called from many places, put `[IgnoreWarning(1371)]` on the `[BurstDiscard]` method itself to suppress the warning at every call site, instead of annotating each caller:
